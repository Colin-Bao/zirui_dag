...
def restart_scheduler():
    # 重启进程
    restart_cmd = 'bash /home/lianghua/rtt/soft/airflow/dags/zirui_dag/sh_files/restart_scheduler.sh'
    logging.warning(f'正在重启scheduler:{restart_cmd}')
    subprocess.Popen(
        restart_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def check_process(Process):
    ...
    if res.stdout.read().decode('utf-8').strip() == '0':
        ...
        msg = ['%s 已停止运行,日志文件详见scheduler_out.log' % Process]
        logging.warning(msg)
        ...
        msg = ['%s 正常运行,日志文件详见scheduler_out.log'% Process]
        logging.info(msg)
        send_mail(USER_NAME, USER_PWD,
                  USER_HOST, SEND_TO, subject, msg)
        logging.info(f'已发送邮件:{SEND_TO} {subject} {msg}')
